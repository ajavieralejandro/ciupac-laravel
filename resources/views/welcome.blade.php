...
#mapid { height: 200px; width: 100vh; }
...
<body class="bg-white-200		">
  ...
  <main class="h-screen w-screen py-6  flex items-center justify-center flex-wrap">
    ...
    @foreach($posts as $post) <div class="bg-white w-72 h-96 shadow-md rounded m-3">
      <div class="h-3/4 w-full">
        <img class="w-full h-full object-cover rounded-t" src="{{asset($post->image_path.'/'.$post->image_name)}}" alt="piña">
      </div>
      <div class="w-full h-1/4 p-3">
        <a href="#" class=" hover:text-yellow-600 text-gray-700">
          <span class="text-lg font-semibold uppercase tracking-wide ">{{$post->title}}</span>
        </a>
        <p class="text-gray-600 text-sm leading-5 mt-1">{{$post->description}}</p>
