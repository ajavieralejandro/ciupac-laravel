                        ...
                        <ul class="max-w-xs flex flex-col divide-y divide-gray-200 dark:divide-gray-700  ">
                            @foreach($stations as $stationA)
                                ...
                                <li
                                    ...
                                    <a
                                        href="{{ route('showStation', ['id' => $stationA->id]); }}">{{ $stationA->name }}</a>
                                </li>
                            @endforeach
                            {{ $stations->links() }}
                        ...
                        <div
                            ...
                            <h2 class="text-md mb-2 px-4 pt-2">
                                <div class="flex justify-between">
                                    <div class="badge relative top-0">
                                        <span
                                            class="mt-2 py-1 h-12px text-md font-semibold   rounded right-1 bottom-1 px-4">{{ $station->name }}</span>
                                    </div>
                            ...
                            <div class="text-md pt-4 pb-4 px-4">
                                <div class="float-right m-auto">
                                    <h1 class="text-6xl"> {{ $temperature }} ºC </h1>
                                </div>
                                <div class="flex justify-between items-center">
                                    <div class="space-y-2">

                                        <span class="flex space-x-2 items-center">UVI : {{ $uvi }}</span>
                                        <span class="flex space-x-2 items-center">Precipitación diaria :
                                            {{ $rain }}</span>

                                        <span class="flex space-x-2 items-center">Punto de rocio: {{ $rocio }} ºC</span>
                                        <span class="flex space-x-2 items-center">rafaga máxima: {{ $wind_gust }}
                                            km/h</span>

                                        <span class="flex space-x-2 items-center">Presión Relativa : {{ $pressureR }}
                                        </span>
                                        <span class="flex space-x-2 items-center">Presión Absoluta :
                                            {{ $pressureA }}</span>




                                        <span class="flex space-x-2 items-center">Viento : <svg height="20" width="20"
                                                viewBox="0 0 32 32" class="fill-current">
                                                <path
                                            ...
                                            </svg> <span>{{ $wind }} km/h</span></span><span
                                            class="flex space-x-2 items-center">Humedad : <svg height="20" width="20"
                                                viewBox="0 0 32 32" class="fill-current">
                                                <path
                                            ...
                                            </svg> <span>{{ $humidity }}%</span></span>
                                        <span class="flex space-x-2 items-center">Sensación términa: {{ $feels_like }}
                                            ºC</span>
                                        @if($report!=null)
...
<script>
    ...
    }).setView([stationA.latitude, stationA.longitude], 7);
    console.log(stationA);
    var display = `<a href="/estacion/${stationA.id}">${stationA.name}</a>`;
    var marker = L.marker([stationA.latitude, stationA.longitude]).addTo(map);
